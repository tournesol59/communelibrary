...
if ((!isset($postData['name'])) || (!isset($postData['email'])) || (!isset($postData['publication'])) || (!isset($postData['birthdate'])) || (!isset($postData['cityaddress'])) || (!isset($postData['option'])) || (!isset($postData['password'])) || (!isset($postData['repassword']))) {
   $statusUser = "error";
   ...
   $cityaddress = $postData['cityaddress'];
   $contact1 = $postData['contact1'];
   $lastuserid = 1; // set in the following insert operation
   if ($password === $repassword) {  // fred do other sanitizer in the future
     ...
     try{
      ...
      $query->execute();
      $lastuserid = $_connection->lastInsertId();
     ...
     try {
       ...
       $_connection->exec("set names utf8");
       $_connection->setAttribute(\PDO::ATTR_ERRMODE, \PDO::ERRMODE_EXCEPTION);
       $sql = "INSERT INTO address (userid,city) VALUES (?,?)";
       $query = $_connection->prepare($sql);
       $query->bindValues($lastuserid, $cityaddress);
       $query->execute();
...
<?php if (!isset($loggedUser['login'])) : ?>
	...
	<?php if (isset($errorMessage)) : ?>
	    ...
	    <div class="mb-3">
            ...
            <div class="mb-3">
                <label for="cityaddress" class="form-label">Votre adresse (ville seulement)</label>
                <input type="text" class="form-control" id="cityaddress" name="cityaddress">
	    </div>
		...
		<select name="option" id="option">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" placeholder="page perso" id="publication" name="publication"></textarea>
	    </div>
            <div class="mb-3">
		 <input type="checkbox" id="contact1" name="contact1" value="true" checked>
                 <label for="contact1">Je suis d accord pour etre un contact</label><br>
            </div>      
